...
class PortfolioAnalyzer:
    ...
    def calculate_yearly_returns(self, historical_performance: pd.DataFrame) -> pd.DataFrame:
        """Compute per-calendar-year total return from cumulative portfolio performance"""
        if historical_performance.empty or 'Total_Value' not in historical_performance.columns:
            return pd.DataFrame()
        df = historical_performance[['Total_Value']].copy()
        ...
        for year in sorted(df.index.year.unique()):
            ...
            if start_val > 0:
                results.append({'Year': year, 'Return_Pct': (end_val / start_val - 1) * 100})
        return pd.DataFrame(results).set_index('Year') if results else pd.DataFrame()

    def create_yearly_returns_chart(self, yearly_returns: pd.DataFrame, title: str = "Annual Total Returns") -> go.Figure:
        """Bar chart of per-year returns with mean, median and ±1σ band"""
        if yearly_returns.empty:
            fig = go.Figure()
            fig.add_annotation(text="Need ≥2 years of data", xref="paper", yref="paper",
                               x=0.5, y=0.5, showarrow=False, font=dict(size=16))
            return fig
        years_list = yearly_returns.index.astype(str).tolist()
        returns = yearly_returns['Return_Pct'].values
        ...
        fig.add_hrect(
            ...
            annotation_text=f"±1σ = {std_val:.1f}%", annotation_position="top right",
            annotation_font_color='#3498db'
        )
        fig.add_trace(go.Bar(
            x=years_list, y=returns, marker_color=colors,
        ...
        fig.add_hline(
            y=mean_val, line_dash='solid', line_color='#3498db', line_width=2.5,
            annotation_text=f"Mean {mean_val:+.1f}%", annotation_position="bottom right",
            annotation_font_color='#3498db', annotation_font_size=12
        )
        fig.add_hline(
            y=median_val, line_dash='dash', line_color='#9b59b6', line_width=2,
            annotation_text=f"Median {median_val:+.1f}%", annotation_position="top left",
            annotation_font_color='#9b59b6', annotation_font_size=12
        )
        fig.update_layout(
...
def main():
    ...
    if '_analysis' in st.session_state:
        ...
        tab1, tab2, tab3 = st.tabs(["🎯 Sharpe Coefficient Breakdown", "🎯 Diversification Score Breakdown", "📅 Annual Return Stability"])
        ...
        with tab3:
            st.subheader("📅 Portfolio Annual Return Stability")
            st.info("📊 Year-by-year total return (price + dividends) — reveals how consistent performance has been over the analysis window")

            portfolio_yearly = analyzer.calculate_yearly_returns(historical_performance)

            if not portfolio_yearly.empty and len(portfolio_yearly) >= 2:
                yr_fig = analyzer.create_yearly_returns_chart(
                    portfolio_yearly,
                    title=f"Portfolio Annual Total Return — {years}-Year Window"
                )
                st.plotly_chart(yr_fig, use_container_width=True)

                returns_arr = portfolio_yearly['Return_Pct'].values
                col1, col2, col3, col4, col5 = st.columns(5)
                with col1:
                    st.metric("Mean / Year", f"{np.mean(returns_arr):+.1f}%")
                with col2:
                    st.metric("Median / Year", f"{np.median(returns_arr):+.1f}%")
                with col3:
                    st.metric("Std Dev (σ)", f"{np.std(returns_arr):.1f}%",
                              help="Lower σ = more consistent returns year to year")
                with col4:
                    best_yr = portfolio_yearly['Return_Pct'].idxmax()
                    st.metric("Best Year", f"{portfolio_yearly.loc[best_yr, 'Return_Pct']:+.1f}%",
                              delta=str(best_yr))
                with col5:
                    worst_yr = portfolio_yearly['Return_Pct'].idxmin()
                    st.metric("Worst Year", f"{portfolio_yearly.loc[worst_yr, 'Return_Pct']:+.1f}%",
                              delta=str(worst_yr))

                pos_years = int((portfolio_yearly['Return_Pct'] > 0).sum())
                total_yr_count = len(portfolio_yearly)
                st.info(
                    f"📈 **{pos_years}/{total_yr_count} positive years** "
                    f"({pos_years / total_yr_count * 100:.0f}% win rate) — "
                    f"note: the first and last calendar years in the window may be partial years."
                )
            else:
                st.warning("Need at least 2 full calendar years of data. Select a longer analysis period (3y, 5y, or 10y).")

            st.markdown("---")
            st.caption("📊 Individual stock yearly return charts are in the **Comprehensive Holdings Analysis** section below — use the stock dropdown there.")
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Yearly_Returns_List": st.column_config.BarChartColumn(
                    "Yr Returns", y_min=-60, y_max=80, width="small",
                    help="Year-by-year total return sparkline — click 'Annual Return Stability' tab for full chart with stats"
                )
            },
            use_container_width=True,
            hide_index=True
        ...
        # Individual Stock Annual Return Breakdown
        st.markdown("---")
        st.subheader("📅 Individual Stock Annual Return Breakdown")

        stocks_with_yearly = {
            s: d for s, d in individual_stock_data.items()
            if d.get('yearly_returns') and len(d['yearly_returns']) >= 2
        }

        if stocks_with_yearly:
            selected_stock = st.selectbox(
                "Select a stock to view its year-by-year return:",
                options=sorted(stocks_with_yearly.keys()),
                key="stock_yearly_selector"
            )
            if selected_stock and selected_stock in stocks_with_yearly:
                yr_dict = stocks_with_yearly[selected_stock]['yearly_returns']
                yr_df = pd.DataFrame(
                    {'Return_Pct': list(yr_dict.values())},
                    index=pd.Index(list(yr_dict.keys()), name='Year')
                ).sort_index()

                col1, col2 = st.columns([3, 1])
                with col1:
                    stock_yr_fig = analyzer.create_yearly_returns_chart(
                        yr_df,
                        title=f"{selected_stock}: Annual Total Return (Price + Dividends)"
                    )
                    st.plotly_chart(stock_yr_fig, use_container_width=True)
                with col2:
                    yr_arr = yr_df['Return_Pct'].values
                    st.markdown(f"**{selected_stock} Stats**")
                    st.metric("Mean / Year", f"{np.mean(yr_arr):+.1f}%")
                    st.metric("Median / Year", f"{np.median(yr_arr):+.1f}%")
                    st.metric("Std Dev (σ)", f"{np.std(yr_arr):.1f}%")
                    best_y = yr_df['Return_Pct'].idxmax()
                    worst_y = yr_df['Return_Pct'].idxmin()
                    st.metric("Best Year", f"{yr_df.loc[best_y, 'Return_Pct']:+.1f}% ({best_y})")
                    st.metric("Worst Year", f"{yr_df.loc[worst_y, 'Return_Pct']:+.1f}% ({worst_y})")
                    pos_yrs = int((yr_df['Return_Pct'] > 0).sum())
                    st.metric("Win Rate", f"{pos_yrs}/{len(yr_df)} years")
        else:
            st.info("No per-stock yearly return data available (need ≥2 years of history per stock).")
